...
async function main() {
    ...
    /* El nadir: la zona que el vuelo no alcanza.
       
       La foto llega hasta unos 4 grados del nadir; esas ultimas filas no
       existen y hay que poner algo. Se repite la ultima fila real y se acaba
       ahi.

       Nada de rellenos elaborados. Se probaron siete —color liso, espejo,
       estirado, desenfoque, vegetacion repetida, promedio por fila y satelital
       reproyectada— y todos se ven, porque todos meten en el medio del plano
       algo que no es la foto. La foto emborronada del propio stitch se nota
       menos que cualquier parche, porque es el mismo color, la misma textura y
       la misma luz. Lo unico que sobra es la costura, y para eso alcanza con
       repetir la ultima fila. */
    ...
    for (let y = ultimaReal + 1; y < ALTO; y++) {
        px.copyWithin(y * W * C, ultimaReal * W * C, (ultimaReal + 1) * W * C)
    }
    console.log(`Nadir: se repite la fila ${ultimaReal} en las ultimas ${ALTO - ultimaReal - 1} (${((ALTO - ultimaReal - 1) / ALTO * 180).toFixed(1)} grados sobre el nadir)`)
