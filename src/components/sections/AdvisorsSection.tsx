    ...
    <section className={styles.advSection}>
      <div className={styles.dirContainer}>
        <div className={styles.sectionHeader}>
          ...
          >
            Tu conexión <span className={styles.textGold}>directa</span>
          </motion.h2>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
        ...
        <div className={styles.advGrid}>
          {advisors.map((advisor, i) => (
            <motion.div
              ...
              style={{ height: '100%' }}
            >
              <div className={styles.advCardInner}>
                <div className={styles.advImageWrap}>
                  <Image
                    ...
                    height={500}
                  />
                  <div className={styles.advGradient} />
                </div>

                <div className={styles.advContent}>
                  <span className={styles.advRole}>{advisor.role}</span>
                  <h3 className={styles.advName}>{advisor.name}</h3>
                  <p className={styles.advDesc}>{advisor.description}</p>

                  <a
                    href={advisor.whatsapp}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`${styles.advBtn} crm-track-click`}
                    data-crm-name={`WhatsApp Asesor - Seccion Asesores - ${advisor.name}`}
